...
function CreateAccessory() {
    ...
    const handleSubmit = async (event) => {
        ...
        if (isEdit) {
            try {
                await axios.patch(
                    `${process.env.REACT_APP_API_BASE_URL}/admin/update-accessory/${item._id}`,
                    formData,
                    {
                        headers: {
            ...
            try {
                // const response = 
                await axios.post(`${process.env.REACT_APP_API_BASE_URL}/phu-kien/create`, formData, {
                    headers: {
                        'Content-Type': 'multipart/form-data'
                    }
                });
                            ...
                            <img
                                src={
                                    typeof imgPhuKien === 'string'
                                        ? `${process.env.REACT_APP_API_BASE_URL.replace('/api/v1', '')}/uploads/${imgPhuKien}`
